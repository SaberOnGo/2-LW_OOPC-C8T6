...
void LCD1602_HardwareInit(void)
{
    ...
    // 打开 LCD 电源
	GPIO_InitStructure.GPIO_Pin = LCD_PWR_SW_PIN;
	GPIO_Init(LCD_PWR_SW_PORT, &GPIO_InitStructure);
    LCD_PWR_SW_L();
...
void LCD1602_WriteData(uint8_t data)
{
    ...
    LCD_RS_L();
    delay_us(45);                           // write data to RAM takes about 43us
}
...
void LCD1602_WriteCmd(uint8_t cmd)   // 写命令
{
    ...
    delay_us(45);              // typical command takes about 39us
}
